...
nu=2;          %no of active transmit antennas
...
SpatialBits = floor ( log2 ( nchoosek (Nt , nu ) ) ) ; 
...
All_Ant_Comb = flipud ( nchoosek ( 1 :Nt , nu ) ) ;
...
PossibleAnt_Ind = zeros (nu ,2^ no ) ;

for i = 1 : nu
  PossibleAnt_Ind ( i , : ) = sub2ind ( [Nt 2^ no ] , Possible_Ant_Comb ( : , i ) .' , 1:2^ no ) ;
end

%%% GSM CONSTELLATION DIAGRAM %%%
GSM_Cons_Diagram = zeros (Nt ,2^ no) ;
GSM_Cons_Diagram( PossibleAnt_Ind ) = repmat ( S_diag( PossibleSigSymb ) .' , nu , 1 ) / sqrt ( nu );
...
H=H_cap;
    ...
    BER_SMT_ML =(sum(dec2bin(pregendata,eta)~=ML_Binary_Results))/eta; 
       ...
       SER_GSM(xx)=SER_GSM(xx)+BER_SMT_ML;
...
semilogy(SNRdB,SER_GSM,'yo-','LineWidth',2);

ylabel('Symbol Error Rate') 
xlabel('Average Eb/No,dB');
legend('ESM, correlation','ESM, no correlation','SM no correlation','GSM no correlation');
title('SER vs SNR for 8x4 system under Nakagami Channel, m=1, 11 bpcu');
axis([0 50 10^(-5) 1]);
