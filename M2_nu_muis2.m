...
for xx = 1:length(SNRdB)
    for i = 1:NUM
        ...
        %% eta-mu H_cap (NR x Nt)
        
       % generate eta?mu distributed channel coefficients
       % with integer values of 2*mu and all real positive values of eta
       % generate coefficients with uniformly distributed phase
       coeff=sqrt(eta);
       ni=zeros(Nr,Nt);
       nq=zeros(Nr,Nt);
        for j=1:2*mu
      ...
      H_cap=(sqrt(ni)+sqrt(-nq))/sqrt(2*(mu+eta*mu));
...
title('SER performance of 4 x 4 ESM MIMO system with 16-QAM modulation versus SNR over exact phase eta-mu fading channel ');
axis([0 30 10^(-5) 1]);
hold on;